...
def calculateHKI():
    ...
    for pm10 in listPM10:
        df["HKI"+pm10] = df[pm10].apply(hesapPM10)
    for so2 in listSO2:
        df["HKI"+so2] = df[so2].apply(hesapSO2)
    for no2 in listNO2:
        df["HKI"+no2] = df[no2].apply(hesapNO2)
    for co in listCO:
        df["HKI"+co] = df[co].apply(hesapCO)
...
for sonuc in listsonuc:
    df["HKI"+sonuc] = df[sonuc].apply(hesapsonuc)
